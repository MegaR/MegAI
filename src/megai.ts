...
export class MegAI {
        ...
        do {
            ...
            if (
                ...
                for (const call of toolCalls) {
                    toolOutputs.push(
                        await this.handleToolCall(call, session, update)
                    );
                }
    ...
    private async handleCodeInterpreter(
        threadId: string,
        runId: string,
        session: Session
    ) {
        ...
        for (const step of steps) {
            ...
            for (const call of step.step_details.tool_calls) {
                ...
                session.attachments.push({
                    name: "code.txt",
                    file: Buffer.from(call.code_interpreter.input),
                });
